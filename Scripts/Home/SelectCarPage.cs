...
public class SelectCarPage : MonoBehaviour {
	...
	void ShowCarProperty(){
		...
		PerAS = (float)tempAS / 100;
		if (canPropTSMove) {
			if (tempTS < TopSpeed) {
				tempTS += Time.deltaTime * 400;
				if (tempTS >= TopSpeed)
					canPropTSMove = false;
	...
	void CreateCar(){
		...
		for (int i = 0; i < NumOfCar; i++){
			...
			GameObject a = Instantiate(AllCar[i]) as GameObject;
			...
			if (HaveStar >= GameStaticData.loadCarProperty.CarProperties [i].UnlockNum)
...
*/
		...
		this.transform.GetChild(GameStaticData.SelectedCar).localPosition = camPos;
