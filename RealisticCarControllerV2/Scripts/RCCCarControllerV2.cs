...
public class RCCCarControllerV2 : MonoBehaviour {
	...
	//Downforce.
	public float downForce = 250f;		// Applies Downforce Releated With Vehicle Speed.

	public float speed;		// Vehicle Speed as KM/H.
	public float brake = 3000f;		// Maximum Brake Torque.
	public float maxspeed = 220f;		//Maximum Speed.

	private float resetTime = 0f;
	private float defSteerAngle = 0f;

	private float highSpeedAngle = 5;
	private float lowSpeedAngle = 10;
	// Gears.
	...
	[Range(0f, 2f)]public float maxEngineSoundPitch = 1.75f;
	...
	public Transform WayPointDetect;
	...
	float AccelSpeed;
	float SteerSpeed;
	float tempSteerSpeed;
	void Start (){
		PlayerDetect = this.transform.FindChild("LookPt");
		WayPointDetect = this.transform.FindChild ("Helpers").FindChild ("WaypointTargetObject");
		wpt = GetComponent<WaypointProgressTracker> ();
		...
		brake = 100000 * GameStaticData.loadCarProperty.CarProperties [GameStaticData.SelectedCar].Brake / 100.0f;
		AccelSpeed = GameStaticData.loadCarProperty.CarProperties [GameStaticData.SelectedCar].Accel / 100.0f;
		SteerSpeed = GameStaticData.loadCarProperty.CarProperties [GameStaticData.SelectedCar].Steer / 100.0f;
		tempSteerSpeed = SteerSpeed;
	}
				...
				if (!wpt.isFlyRes) {
					if (speed <= 60) {
						...
						if (!GameStaticData.isAddSpeed) {
							...
							rigid.AddForce (transform.forward * (15 - speedlevel) * AccelSpeed, ForceMode.Acceleration);
							
							speedShake.Instance.Shake (0f,1f);
							speedShake1.Instance.Shake (0f,1f);
							speedShake2.Instance.Shake (0f, 1f);


						} else {
							rigid.AddForce (transform.forward * 200, ForceMode.Acceleration);
						
							if (!GameStaticData.isVR)
								speedShake2.Instance.Shake (0.025f, 1f);
							else {
								speedShake.Instance.Shake (0.025f, 1f);
								speedShake1.Instance.Shake (0.025f, 1f);
							}
							print ("hi");
						}
					}
	...
	public float pitchValMin;
	public float pitchValMax;
	public float diffPithcVal;
	...
	public void ApplyEngineSound(float input){
		...
		if(engineSoundOn){
			...
			if(automaticGear){
				...
				if (!reverseGear) {
				//	print ("VolumeSpeed : "+VolumeSpeed);
					//engineSoundOn.volume = Mathf.Lerp (engineSoundOn.volume, Mathf.Clamp (gasInput, minEngineSoundVolume * fuelInput * GameStaticData.MusicValue, maxEngineSoundVolume * GameStaticData.MusicValue), Time.deltaTime * 50f);
					engineSoundOn.volume = Mathf.Lerp (engineSoundOn.volume, Mathf.Clamp (speed/maxspeed, minEngineSoundVolume * fuelInput * GameStaticData.MusicValue, maxEngineSoundVolume * GameStaticData.MusicValue), Time.deltaTime * 20f);
				
				}else
					engineSoundOn.volume = Mathf.Lerp (engineSoundOn.volume, Mathf.Clamp (brakeInput, minEngineSoundVolume * fuelInput *GameStaticData.MusicValue, maxEngineSoundVolume *GameStaticData.MusicValue), Time.deltaTime * 20f);

			}else{

				engineSoundOn.volume = Mathf.Lerp (engineSoundOn.volume, Mathf.Clamp (gasInput, minEngineSoundVolume * fuelInput *GameStaticData.MusicValue, maxEngineSoundVolume *GameStaticData.MusicValue), Time.deltaTime * 20f);

			}
			pitchVal = (speed / maxspeed) * 1.5f;
			//pitchVal = Mathf.Clamp (pitchVal,Mathf.Clamp (pitchValMin,0,2.5f),Mathf.Clamp (pitchValMax,1,3));
			//print ("pitchVal : "+ pitchVal +" | pitchValMin : " + pitchValMin + " | pitchValMax : " + pitchValMax);
			pitchVal = Mathf.Clamp (pitchVal,0.5f,1.5f);


			if (engineRunning) {
				
				engineSoundOn.pitch = Mathf.Lerp (engineSoundOn.pitch,pitchVal,Time.deltaTime*20);
			}
			/*

			if(engineRunning)
				engineSoundOn.pitch = Mathf.Lerp ( engineSoundOn.pitch, Mathf.Lerp (minEngineSoundPitch * fuelInput, maxEngineSoundPitch, (engineRPM) / (maxEngineRPM)), Time.deltaTime * 20f);
			else
				engineSoundOn.pitch = Mathf.Lerp ( engineSoundOn.pitch, 0, Time.deltaTime * 20f);
*/
		...
		if(engineSoundOff){
			...
			if(automaticGear){
				...
				if(!reverseGear)
					engineSoundOff.volume = Mathf.Lerp (engineSoundOff.volume, Mathf.Clamp (((1 + (-gasInput)) * fuelInput), minEngineSoundVolume * fuelInput *GameStaticData.MusicValue, .5f *GameStaticData.MusicValue), Time.deltaTime * 50f);
				else
					engineSoundOff.volume = Mathf.Lerp (engineSoundOff.volume, Mathf.Clamp (((1 + (-brakeInput)) * fuelInput), minEngineSoundVolume * fuelInput *GameStaticData.MusicValue, .5f *GameStaticData.MusicValue), Time.deltaTime * 50f);
	...
	public void Braking (){
		...
		if(handbrakeInput > .1f){
			...
			ApplyBrakeTorque(RearLeftWheelCollider, (brake * 5.5f) * handbrakeInput);
			ApplyBrakeTorque(RearRightWheelCollider, (brake * 5.5f) * handbrakeInput);
	...
	public void AntiRollBars (){
		...
		if (groundedFL) {
			...
			if (GameStaticData.checkFly) {
				StartCoroutine(DelayMotor());
				GameStaticData.checkFly = false;
	...
	public void DownForce(){
		rigid.AddForce(-transform.up * (downForce * rigid.velocity.magnitude) * 5f);
	}
	...
	public float an1 = 0;
	public float min1 = 0;
	public float max1 = 0.29f;

	public float an2 = 1.2f;
	public float min2 = 0.29f;
	public float max2 = 0.45f;

	public float an3 = 1.8f;
	public float min3 = 0.45f;
	public float max3 = 0.9f;

	public float an4 = 2.1f;
	public float min4 = 0.9f;
	public float max4 = 1;
	...
	public void KeyboardControlling (){
		...
		int num;
			...
			if (Contorl_Example.BLE_LB || Input.GetKey (KeyCode.Space)) {
				handbrakeInput = 1;
				rigid.drag = 0.45f;
				this.transform.FindChild ("brake").gameObject.SetActive (true);

			} else {
				handbrakeInput = 0;
				rigid.drag = 0.1f;

				this.transform.FindChild ("brake").gameObject.SetActive (false);
		...
		if (GameStaticData.isStart) {
			if (Contorl_Example.BLE_RB) {
				gasInput = Mathf.Lerp (gasInput, 1, Time.deltaTime * 10f);
			} else {
		...
		//	Vector3 cameraRelative = PlayerDetect.InverseTransformPoint(WayPointDetect.position);
		//		print (cameraRelative.x);

		/*if (cameraRelative.x > 5 &&( Mathf.Abs (Contorl_Example.BLE_aY) < 0.3f || Mathf.Abs (Input.GetAxis("Horizontal")) < 0.3f)) {
			//	print ("Left");
			steerInput = Mathf.Lerp (steerInput, 0.1f, Time.deltaTime * 20f);
		} else if (cameraRelative.x < -5 && ( Mathf.Abs (Contorl_Example.BLE_aY) < 0.3f || Mathf.Abs (Input.GetAxis("Horizontal")) < 0.3f) ) {
//			print ("Right");
			steerInput = Mathf.Lerp (steerInput, -0.1f, Time.deltaTime * 20f);
		} else {
...
*/
		...
		if(Mathf.Abs(Input.GetAxis("Horizontal")) > 0.05f)
			steerInput = Mathf.Lerp (steerInput, Input.GetAxis("Horizontal") * 0.7f, Time.deltaTime * 20f);
		else
			steerInput = Mathf.Lerp (steerInput, Input.GetAxis("Horizontal") * 0.7f, Time.deltaTime * 20f);
		

		if (Mathf.Abs (Contorl_Example.BLE_aY) > 0.05f) {
			GameStaticData.isContorl = true;
		} else {
			GameStaticData.isContorl = false;
		}

		if (Contorl_Example.BLE_aY > 0.05f) {

			if (Contorl_Example.BLE_aY > 0.f) {
				if (Contorl_Example.BLE_aY > 0.7f) {
					Contorl_Example.BLE_aY = 0.7f;
				}		
			}
			steerInput = Mathf.Lerp (steerInput, Contorl_Example.BLE_aY , Time.deltaTime * 200f);

		} else if (Contorl_Example.BLE_aY < -0.05f) {
			if (Contorl_Example.BLE_aY < -0.5f) {
				if (Contorl_Example.BLE_aY < -0.7f) {
					Contorl_Example.BLE_aY = -0.7f;
				}
...
*/
		/*
		if (Mathf.Abs (Contorl_Example.BLE_aY) > 0.8f) {
			riri = 4;
		} else if (Mathf.Abs (Contorl_Example.BLE_aY) > 0.55f) {
			riri = 3;
		} else if (Mathf.Abs (Contorl_Example.BLE_aY) > 0.3f) {
			riri = 2;
		} else if (Mathf.Abs (Contorl_Example.BLE_aY) > 0.15f) {
			riri = 1.1f;
		}else {
			riri = 0;
		}
		...
		FrontLeftWheelCollider.steerAngle = riri * ppp;
		FrontRightWheelCollider.steerAngle =  riri * ppp;
		if (riri == 4) {
			handbrakeInput = 0.5f;
		}
		...
		*/
		...
		if (Mathf.Abs (Contorl_Example.BLE_aY) >= min1 && Mathf.Abs (Contorl_Example.BLE_aY) < max1) {
			riri = an1;
		}else if (Mathf.Abs (Contorl_Example.BLE_aY) >= min2 && Mathf.Abs (Contorl_Example.BLE_aY) < max2) {
			riri = an2;

		}else if (Mathf.Abs (Contorl_Example.BLE_aY) >= min3 && Mathf.Abs (Contorl_Example.BLE_aY) < max3) {
			riri = an3;

		}else if (Mathf.Abs (Contorl_Example.BLE_aY) >= min4 && Mathf.Abs (Contorl_Example.BLE_aY) < max4) {
			riri = an4;
		...
		*/
		/*
		float speedFactor = rigid.velocity.magnitude / gameManagerBehaviour.Car_Topspeed;
		float currentSteerAngel = Mathf.Lerp (lowSpeedAngle, highSpeedAngle, speedFactor);
		currentSteerAngel *= Contorl_Example.BLE_aY;


		FrontLeftWheelCollider.steerAngle = currentSteerAngel;
		FrontRightWheelCollider.steerAngle = currentSteerAngel;
		*/
		if (!doWater) {
			if (Contorl_Example.BLE_aY > 0 && speed > 10) {
				transform.Rotate (new Vector3 (0, Mathf.Clamp (Mathf.Pow ((Contorl_Example.BLE_aY), 2) * 10, 0, 8f) * SteerSpeed, 0) * Time.deltaTime * 10);
			} else if (Contorl_Example.BLE_aY < 0f && speed > 10) {
				transform.Rotate (new Vector3 (0, -(Mathf.Clamp (Mathf.Pow ((Contorl_Example.BLE_aY), 2) * 10, 0, 8f) * SteerSpeed), 0) * Time.deltaTime * 10);
			}
			if (Mathf.Abs (Input.GetAxis ("Horizontal")) > 0.05f && speed > 10)
				transform.Rotate (new Vector3 (0, Input.GetAxis ("Horizontal") * 7 * SteerSpeed, 0) * Time.deltaTime * 10);
		} else {
			if(LR)
				transform.Rotate (new Vector3 (0, 20, 0) * Time.deltaTime * 10);
			else
				transform.Rotate (new Vector3 (0, -20, 0) * Time.deltaTime * 10);
		}
		if(isInWater) {
		...
		#if UNITY_EDITOR
		FrontLeftWheelCollider.steerAngle = Input.GetAxis("Horizontal")*60;

		FrontRightWheelCollider.steerAngle = Input.GetAxis("Horizontal")*60;
		#endif
*/
		if (Mathf.Abs(steerInput) > 0.7f) {
			handbrakeInput = 1;
			print ("brakeInput");
		}
	...
	void OnTriggerEnter(Collider other){
		...
		waterTag = other.tag;
		if (other.transform.CompareTag ("waterrotateL") || other.transform.CompareTag ("waterrotateR") || other.transform.CompareTag ("waterrotateS")) {
			
			//SteerSpeed = 0.2f;
			isInWater = true;
			doWater = true;
		}
		if (other.transform.CompareTag ("mudslow")) {

			onMud = true;
		}
		if (other.transform.CompareTag ("fireslow")) {

			onMud = true;
			FrontLeftWheelTransform.GetChild (0).gameObject.SetActive (true);
			FrontRightWheelTransform.GetChild (0).gameObject.SetActive (true);
			RearLeftWheelTransform.GetChild (0).gameObject.SetActive (true);
			RearRightWheelTransform.GetChild (0).gameObject.SetActive (true);

			transform.FindChild ("Fire2D").gameObject.SetActive (true);
	...
	void OnTriggerExit(Collider other){
		...
		if (other.transform.CompareTag ("fireslow")) {
			...
			onMud = false;
			FrontLeftWheelTransform.GetChild (0).gameObject.SetActive (false);
			FrontRightWheelTransform.GetChild (0).gameObject.SetActive (false);
			RearLeftWheelTransform.GetChild (0).gameObject.SetActive (false);
			RearRightWheelTransform.GetChild (0).gameObject.SetActive (false);
			transform.FindChild ("Fire2D").gameObject.SetActive (false);
	...
	void OnCollisionEnter (Collision collision){
		...
		if(crashClips.Length > 0){
			if (collision.contacts[0].thisCollider.gameObject.transform != transform.parent){
				...
				if(!collision.transform.CompareTag("Car") && !collision.transform.CompareTag("Road"))
					rigid.velocity = 60f *((speed/2)/140) * rigid.velocity.normalized;
//				print ("rigid.velocity : "+ rigid.velocity);
	...
	public void Lights (){
		...
		if(Input.GetKeyDown(KeyCode.H) && !AIController){
