	...
	public class CarAIControl : MonoBehaviour
	{
		...
		private void FixedUpdate()
		{
			...
			else
			{
				...
				float targetAngle = Mathf.Atan2(localTarget.x, localTarget.z)*Mathf.Rad2Deg*addturn;
...
//				if(NeedBack)
//				if(m_CarController.speed <= 0.5f){
//					NeedBack = true;
//					float back;
//					back += Time.deltaTime;
//					m_CarController.Move(steer, -1, -1, 0f,0);
//
//					if (back >= 3)
//						NeedBack = false;
//				}
				...
				if (isStart) {
					if(CanAdd)
					//m_Rigidbody.AddForce (transform.forward * 25, ForceMode.Acceleration);

						if (NeedTurn) {

							if (m_CarController.speed < 70) {
								if (LR == 1) {
									m_CarController.Move (-0.125f, accel * 0.15f, accel * 0.15f, 1f, 0);
								} else if (LR == 2) {
									m_CarController.Move (0.125f, accel * 0.15f, accel * 0.15f, 1f, 0);
								}
							} else {
								if (LR == 1) {
									m_CarController.Move (-0.1f, accel * 0.15f, accel * 0.15f, 1f, 0);
								} else if (LR == 2) {
									m_CarController.Move (0.1f, accel * 0.15f, accel * 0.15f, 1f, 0);
								}
						...
						if (this.transform.parent.name != "1") {
							m_CarController.Move (steer, aiSpeed, aiSpeed, 0f, 0);
						} else {
...
//                
		...
		void OnTriggerEnter(Collider other){
			...
			if (other.transform.CompareTag ("waterrotateR")) {



			}
			if (other.transform.CompareTag ("mudslow")||other.transform.CompareTag ("fireslow")) {
				onMud = true;
				m_Rigidbody.drag = 1.5f;
				StartCoroutine (aiMud());
